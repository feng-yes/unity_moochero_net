...
public class ZombieAI : MonoBehaviourPunCallbacks {
	...
	void OnEnable()
	{
		...
		agent = GetComponent<UnityEngine.AI.NavMeshAgent>();			//获取NavMeshAgent组件
	...
	public void requestDisable(){
		photonView.RPC ("DisableZombie", RpcTarget.All);
	}
	//RPC函数，禁用所有客户端的将是对象
	[PunRPC]
	void DisableZombie()
	...
	void UpdateWanderState()
	{
		...
		if (zombieRender != null && zombieRender.isCrazy == true)
			photonView.RPC ("ZombieSetNormal", RpcTarget.All);
	}
	...
	void UpdateTrackState()
	{
		...
		if (zombieRender != null && zombieRender.isCrazy == false)
			photonView.RPC ("ZombieSetCrazy", RpcTarget.All);
	}
		...
		if (ph != null)
		{
			...
			if (degree < attackFieldOfView / 2 && degree > -attackFieldOfView / 2) {
				...
				if (attackTime > attackInterval) {
					...
					photonView.RPC ("PlayZombieAttackAudio", RpcTarget.All);
				}
	...
	{
		photonView.RPC ("ZombieDead", RpcTarget.All);
