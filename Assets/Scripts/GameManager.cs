...
public class GameManager : MonoBehaviourPunCallbacks {
	...
	void Start () {
		...
		photonView.RPC ("ConfirmLoad", RpcTarget.All);				//使用RPC,告知所有玩家有一名玩家已成功加载场景
		...
		case GameState.Playing:				
			...
			if (PhotonNetwork.IsMasterClient) {					//MasterClient检查游戏状态
				if (currentScoreOfTeam1 >= targetScore)			//队伍1达到目标分数，队伍1获胜
					photonView.RPC ("EndGame", RpcTarget.All, "Team1",PhotonNetwork.Time);
				else if (currentScoreOfTeam2 >= targetScore)	//队伍2达到目标分数，队伍2获胜
					photonView.RPC ("EndGame", RpcTarget.All, "Team2",PhotonNetwork.Time);
	...
	public override void OnPlayerLeftRoom(Player other){
		...
		if (PhotonNetwork.IsMasterClient) {	//MasterClient检查
			...
			//更新玩家得分榜
			photonView.RPC ("UpdateScores", RpcTarget.All, currentScoreOfTeam1, currentScoreOfTeam2);
		}
	}
	...
	void CheckTeamNumber(){
		Player[] players = PhotonNetwork.PlayerList;		//获取房间内玩家列表
		int teamOneNum = 0, teamTwoNum = 0;						
		foreach (Player p in players) {					//遍历所有玩家，计算两队人数
	...
	void CheckPlayerConnected(){
		if (countDown <=0.0f || loadedPlayerNum == PhotonNetwork.PlayerList.Length) {	//游戏开始倒计时结束，或者所有玩家加载场景
			...
			photonView.RPC ("StartGame",RpcTarget.All,startTimer);		//使用RPC，所有玩家开始游戏
		}
	}
	...
	void InstantiatePlayer(){
		playerCustomProperties= PhotonNetwork.LocalPlayer.CustomProperties;	//获取玩家自定义属性
	 ...
	 * 该函数只由MasterClient调用
	 */
	public void AddScore(int killScore, Player p){
	...
	void UpdateScores(int teamOneScore,int teamTwoScore){
		...
		Player[] players = PhotonNetwork.PlayerList;	//获取房间内所有玩家的信息
	...
	void EndGame(string winTeam,double timer){
		...
		else if (winTeam == PhotonNetwork.LocalPlayer.CustomProperties ["Team"].ToString ()) 	//如果玩家属于获胜队伍
		{
