...
public class PlayerHealth : MonoBehaviourPunCallbacks{
	...
	void Start () {
		...
		photonView.RPC ("UpdateHP", RpcTarget.Others, currentHP);				//使用RPC，更新其他客户端中该玩家对象当前血量
		if (PhotonNetwork.LocalPlayer.CustomProperties ["Team"].ToString () == "Team1")	//设置玩家对象队伍
			team = 1;
		else
			team = 2;
		photonView.RPC ("SetTeam", RpcTarget.Others, team);		//使用RPC，设置其他客户端中该玩家对象的队伍
	}
	...
	public void TakeDamage(int damage, Player attacker){
		...
		if (PhotonNetwork.IsMasterClient) {		//MasterClient调用
			...
			photonView.RPC ("UpdateHP", RpcTarget.All, currentHP);	//更新所有客户端，该玩家对象的生命值
	...
	{
		photonView.RPC ("AddHP", RpcTarget.MasterClient, value);	//使用RPC,向MasterClient发起加血请求
	}
	...
	public void AddHP(int value)
	{
		...
		photonView.RPC ("UpdateHP", RpcTarget.All, currentHP);	//使用RPC，更新所有客户端，该玩家对象的血量
	}
	...
	//玩家复活函数
	void PlayerSpawn(){
		photonView.RPC ("PlayerReset", RpcTarget.All);	//使用RPC，初始化复活时的玩家属性
