...
public class PlayerShoot : MonoBehaviourPunCallbacks {
	...
	void Update () {
		...
		if (CrossPlatformInputManager.GetButton("Fire1") && timer >= timeBetweenShooting) {	//玩家按下射击键，与上次射击的间隔超过射击间隔
			...
			if (GameManager.gm.state == GameManager.GameState.Playing)						//如果当前游戏状态处于游戏进行中
				photonView.RPC ("Shoot", RpcTarget.MasterClient, PhotonNetwork.LocalPlayer);	//使用RPC,调用MasterClient的Shoot函数，函数参数为发起射击的玩家
		}
	}
	...
	void Shoot(Player attacker){
		...
		photonView.RPC ("ShootEffect", RpcTarget.All, bulletEffectPosition);//使用RPC，调用所有玩家对象的ShootEffect函数，显示射击效果
	}
